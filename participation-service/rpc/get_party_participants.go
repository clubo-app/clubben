...
func (s server) GetPartyParticipants(ctx context.Context, req *participation.GetPartyParticipantsRequest) (*participation.PagedPartyParticipants, error) {
	...
	pp, p, err := s.pp.GetPartyParticipants(ctx, repository.GetPartyParticipantsParams{
		PId:   req.PartyId,
		Page:  p,
		Limit: int(req.Limit),
	})
